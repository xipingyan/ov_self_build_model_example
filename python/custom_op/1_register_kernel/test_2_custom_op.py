...
def run_ov_model(inputs_pt:list, onnx_model_fn, device='CPU', dynamic_shape=False):
    ...
    if onnx_model_fn is None:
        ...
        if not os.path.exists(f"{TMP_DIR}/export_ov_model"):
            os.mkdir(f"{TMP_DIR}/export_ov_model")
...
class MyAddPyOP2(torch.autograd.Function):
    @staticmethod
    def symbolic(g, x, alpha, beta):
        alpha = torch.tensor(alpha)
        alpha = g.op("Constant", value_t=alpha)

        beta = torch.tensor(beta)
        beta = g.op("Constant", value_t=beta)
        return g.op('MyEltwise', x, alpha, beta)
...
class MyPytorchModel(nn.Module):
    def __init__(self, last_bias, alpha, beta):
        ...
        self.alpha = alpha
        self.beta = beta
        self.my_eltwise_py_op = MyAddPyOP2
    ...
    def forward(self, x):
        ...
        r3 = self.my_eltwise_py_op.apply(r2, self.alpha, self.beta)
        return r3

def main(device, dynamic_shape:bool):
    ...
    model_pt = MyPytorchModel(weight_bias, weight_alpha, weight_beta)
    ...
    print(" == Start to analysis result.")
