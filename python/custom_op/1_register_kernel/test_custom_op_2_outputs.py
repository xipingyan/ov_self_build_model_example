...
def run_ov_model(inputs_pt:list, onnx_model_fn, device='CPU', dynamic_shape=False):
    ...
    for id, input_pt in enumerate(inputs_pt):
        ...
        output=[]
        output.append(copy.deepcopy(out["output0"].tolist()))
        output.append(copy.deepcopy(out["output1"].tolist()))
        outputs.append(output)
    return outputs

class MyAdd2Output(torch.autograd.Function):
...
class MyPytorchModel(nn.Module):
    def __init__(self, last_bias1, last_bias2):
