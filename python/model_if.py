...
def model_if():
    ...
    # mutmal = opset.add(input1, op.Constant(np.full((1), 2).astype(np.int32)))
    ...
    if_op.set_input(add1.output(0), input_then, input_else)
    ...
    return Model(results=[res], parameters=[input1], name='model_if')
...
def test_model_if(device:str):
    ...
    result = infer_request.infer([input1])[compiled_model.output(0)]
