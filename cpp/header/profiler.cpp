...
struct dump_items {
    ...
    std::string file;       // The source file name
...
Profile::Profile(const std::string &name, const char *file, int line, const std::vector<std::pair<std::string, std::string>> &args) : _name(name), _file(file), _line(line), _active(true)
{
    _ts1 = __rdtsc();
}
